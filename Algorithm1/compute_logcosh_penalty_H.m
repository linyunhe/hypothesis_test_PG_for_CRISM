function [o_penalty, d_penalty, new_image] = compute_logcosh_penalty_H (sensitivity_image, spec_pnt,update_factor, image_last_iter, beta_spat, delta_spat, beta_spec, delta_spec, spectral_span, max_iter, spat_penalty_on, spec_penalty_on, use_mex)
tic;

    % Run MEX version if we have it
    if use_mex
        if exist('compute_logcosh_penalty_H_mex','file')
            disp('Running C++ version of compute_logcosh_penalty_H');
            global mex_num_threads;
            if(isempty(mex_num_threads))
                mex_num_threads=-1; % default to max threads
            end
            [o_penalty, d_penalty, new_image] = compute_logcosh_penalty_H_mex(sensitivity_image, spec_pnt,update_factor, image_last_iter, beta_spat, delta_spat, beta_spec, delta_spec, spectral_span, max_iter, spat_penalty_on, spec_penalty_on, mex_num_threads);
            toc;
            return;
        else
        ...
        disp('Running Matlab version of compute_logcosh_penalty_H');
    end
    ...
    if spat_penalty_on == 0 && spec_penalty_on == 0,
        ...
        new_image = em_estimate_scaled./sensitivity_image;%no need to worry about NaN values in the result since they don't propogate during forward and backward projection
        d_penalty = 0;
        o_penalty = 0;
        return;
    end
