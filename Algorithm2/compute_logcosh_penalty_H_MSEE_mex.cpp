...
// 	}
	...
	if ( !(penalty_params.spat_penalty_flag || penalty_params.spec_penalty_flag) ) {
        ...
        #pragma omp parallel for
		for (int i = 0; i < numEls; ++i) {
			(*new_estimate)(i) = (*update_factor)(i) / (*sensitivity_image)(i) + (*image_last_iter)(i);
		}
...
//         tmp = image_last_iter - new_estimate;
		...
		for (int i = 0; i < numEls; ++i) {
			...
			obj_function(i) = (*sensitivity_image)(i) * ((*image_last_iter)(i) - (*new_estimate)(i)) * ((*image_last_iter)(i) - (*new_estimate)(i))  + \
				2 * (*update_factor)(i) * ((*image_last_iter)(i) - (*new_estimate)(i)) + penalty_function_zeroed_ijk;
			obj_gradient(i) = -2*(*sensitivity_image)(i) * ((*image_last_iter)(i) - (*new_estimate)(i)) - \
				2 * (*update_factor)(i) + penalty_gradient_zeroed_ijk;
			obj_hessian(i) = 2*(*sensitivity_image)(i)  +  penalty_heissian_zeroed_ijk;
...
//        obj_function_2 = sensitivity_image.*tmp.^2 + 2*update_factor.*tmp + temp;
        ...
        for (int i = 0; i < numEls; ++i) {
			...
			obj_function_2_ijk = (*sensitivity_image)(i) * ((*image_last_iter)(i) - rsm_new_estimate(i)) * ((*image_last_iter)(i) - rsm_new_estimate(i)) + 2 * (*update_factor)(i) * ((*image_last_iter)(i) - rsm_new_estimate(i)) + new_penalty_function(i);
